...
def execute_monday_rebalance():
    ...
    trade_row = instructions.iloc[0]
    ticker = str(trade_row['ticker'])
    action = str(trade_row['signal']).upper()
    
    print(f"[*] ThorpTraderExtreme Processing Signal -- Ticker: {ticker} | Action Required: {action}")
    
    # 3. Check for any open pending orders from previous attempts to avoid doubling down
    ...
    if open_orders:
        print(f"[!] Target {ticker} already has an active pending order. Aborting execution to prevent chaos.")
        return

    # 4. Fetch your current broker account position
    current_positions = trading_client.get_all_positions()
    holding_ticker = any(pos.symbol == ticker for pos in current_positions)

    # 5. Execute Order Logic based on the payload contract
    if action == "HOLD":
        ...
        print(f"[▲] EXECUTING SYSTEM BUY ORDER FOR: {ticker}")
        # Sizing is fixed at 50 shares for this baseline test
        ...
        print(f"[▼] EXECUTING SYSTEM LIQUIDATION ORDER FOR: {ticker}")
        order_data = MarketOrderRequest(
            symbol=ticker,
            qty=50,  
            side=OrderSide.SELL,
